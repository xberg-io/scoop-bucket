...
This repository holds Scoop app manifests. The security-relevant surface is **distribution integrity**: a manifest pointing at an unexpected URL, a missing or wrong `hash`, or a `pre_install` / `installer` / `post_install` script running unreviewed remote content. Report anything that lets a user install something other than the intended artifact.
